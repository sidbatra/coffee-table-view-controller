...
UITableViewController's current architecture results in bloated and disorganized table view controllers because it becomes a hub for different types of code, including:
...
The UserPresenter class implements CModelPresenterProtocol and wraps around the code to use UserCell for displaying users populated via UsersViewDataSource.
...
The UsersViewController is responsible for view controller logic and can update the presentation of visible cells by passing them the updated object. For example, if a User image is downloaded the UsersViewController can update visible UserCells.
...
The UsersViewController inherits from CTableViewController and primarily contains view logic.
...
The Examples folder contains the Intermediate project which shows how Coffee Table View Controller works with more complex table views. It aims to render this table view:

-- Intermediate example image--
